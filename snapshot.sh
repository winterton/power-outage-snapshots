...
while :; do
  ...
  curl -k -sS --fail --retry 4 --retry-delay 5 -A "$UA" -o "$f" \
...
curl -k -sS --fail -A "$UA" -o "data/$day/storm-modes-$ts.json" \
...
if state=$(curl -k -sS --fail -A "$UA" "$K/stormcenter/api/v1/stormcenters/$INST/views/$VIEW/currentState?preview=false"); then
  slug=$(echo "$state" | jq -r '.data.interval_generation_data')
  curl -k -sS --fail -A "$UA" -o "data/$day/kubra-summary-$ts.json"  "$K/$slug/public/summary-1/data.json" || true
  curl -k -sS --fail -A "$UA" -o "data/$day/kubra-zips-$ts.json"     "$K/$slug/public/thematic-2/thematic_areas.json" || true
